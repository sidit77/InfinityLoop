...
use std::ops::Range;
use crate::types::{Angle, HexPosition};
...
struct HexagonWorld<T> {
    radius: i32,
    elements: Box<[T]>
}

impl<T: Default + Clone> HexagonWorld<T> {

    fn new(radius: u32) -> Self {
        let radius = i32::try_from(radius.checked_sub(1).unwrap()).unwrap();
    ...
    fn index(&self, pos: HexPosition) -> Option<usize> {
        if pos.q().abs() > self.radius || pos.r().abs() > self.radius  || pos.s().abs() > self.radius {
            ...
            let q = sign * pos.q();
            let r = sign * pos.r();
            let index = len / 2 + sign * (q + r * diameter - ((r - 1) * r) / 2);
            Some(index as usize)
        }
    }

    fn get(&self, pos: HexPosition) -> Option<&T> {
