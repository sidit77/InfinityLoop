...
pub fn enable_immersive() -> Result<()> {
    let ctx = ndk_context::android_context();
    let vm = unsafe { jni::JavaVM::from_raw(ctx.vm().cast()) }?;
    let env = vm.attach_current_thread()?;

    let window = env
        .call_method(ctx.context() as jni::sys::jobject, "getWindow", "()Landroid/view/Window;", &[])?.l()?;
    let view = env
    ...
    env.call_method(window_insets_controller, "setSystemBarsBehavior", "(I)V", &[flag_behavior_show_bars_by_swipe])?;
    ...
    env.call_method(window_insets_controller, "hide", "(I)V", &[system_bars])?;
    ...
    env.set_field(window_attributes, "layoutInDisplayCutoutMode", "I", cutout_mode)?;
